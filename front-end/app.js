...
const STORAGE_KEY = "ai_agent_state"; // you can choose any name
...
const router = {
  ...
  if (!isLoggedIn && page !== 'signup' && page !== 'login' && page  !== 'home') {
    page = 'signup';
  }
...
,
  render() {
    ...
    switch(this.currentPage) {
      ...
      case 'signup':content.innerHTML=signup_page(),initSignupPage();break;
      case 'login':content.innerHTML=login_page();initlogin();break;
      default: content.innerHTML = signup_page();
    }
    updateNavbar();
    // Attach logout event
...
function updateNavbar() {
  const isLoggedIn = localStorage.getItem("isLoggedIn");
  const logoutBtn = document.getElementById("logout-btn");
  const loginLink = document.querySelector('a[data-page="signup"]');

  if (isLoggedIn) {
    logoutBtn.classList.remove("hidden");
    if(loginLink) loginLink.classList.add("hidden"); // hide login/signup link
  } else {
    logoutBtn.classList.add("hidden");
    if(loginLink) loginLink.classList.remove("hidden"); // show login/signup link
  }
}
function logout() {
...
function initAdminPage() {
 ...
 function signup_page(){
  return `<div class="fixed inset-0 flex items-center justify-center px-4 m-0 p-0 ">
  <section class="glass bg-white/5 backdrop-blur-md rounded-3xl p-8 w-full max-w-md border border-white/10 shadow-lg">
    
    <!-- Header -->
    <div class="text-center space-y-2 mb-6">
      <p class="text-xs uppercase tracking-widest text-indigo-300">Create your account</p>
      <h2 class="text-3xl font-bold text-white">Start Learning Smarter</h2>
      <p class="text-slate-300 text-sm">Upload notes, get AI summaries, quizzes & track progress</p>
    </div>
    ...
    <form class="space-y-5" id="signup_form">
      ...
      <!-- Full Name -->
      <div>
        <label class="text-sm text-slate-300" for="name">Full Name</label>
        <input
          id="name"
          type="text"
          placeholder="Enter your name"
          class="mt-1 w-full px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-400 focus:outline-none focus:border-indigo-400"
          required
          minlength="3"
          maxlength="50"
          autocomplete="name"
      ...
      <button
        type="submit"
        class="w-full py-2 rounded-lg gradient text-slate-900 font-semibold hover:opacity-90 transition"
      >
        Create Account
      </button>
    </form>
...
<span
  data-page="login"
  class="nav-link text-indigo-300 cursor-pointer hover:underline"
>
 Login
</span>
...
function initSignupPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        showPopup("Sign  successful", "success"); 


        router.navigate("login");
      } else {
       showPopup(data.message || "Signup failed", "fail");     
      }
    } catch (err) {
      console.error(err);
    }
...
function login_page(){
  ...
  <section class="glass bg-white/5 backdrop-blur-md rounded-3xl p-8 w-full max-w-md border border-white/10 shadow-lg">
    ...
    <!-- Header -->
    <div class="text-center space-y-2 mb-6">
      <p class="text-xs uppercase tracking-widest text-indigo-300">Welcome Back</p>
      <h2 class="text-3xl font-bold text-white">Login to Your Account</h2>
      <p class="text-slate-300 text-sm">Access your notes, summaries, quizzes & progress</p>
    </div>
...
function initlogin() {
  const login_form = qs("login_form");
  if (!login_form) return;

  login_form.addEventListener("submit", async (e) => {
    e.preventDefault();

    const name = qs("name_login").value;
    const password = qs("password_login").value;
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.navigate("home");
         ...
         showPopup(data.message || "Login failed", "fail");
